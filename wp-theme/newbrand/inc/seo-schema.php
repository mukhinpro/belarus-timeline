 ...
 */
function newbrand_seo_plugin_active(): bool {
 ...
 */
function newbrand_business_node(): array {
	$areas = array_filter( array_map( 'trim', explode( ',', newbrand_setting( 'newbrand_areas' ) ) ) );

	$node = [
		'@type'       => [ 'LocalBusiness', 'ProfessionalService' ],
		'@id'         => home_url( '/#business' ),
		'name'        => newbrand_setting( 'newbrand_studio_name' ),
		'description' => get_bloginfo( 'description' ),
		'url'         => home_url( '/' ),
		'priceRange'  => newbrand_setting( 'newbrand_price_range' ),
		'address'     => [
			'@type'           => 'PostalAddress',
			'addressLocality' => newbrand_setting( 'newbrand_city' ),
			'addressRegion'   => newbrand_setting( 'newbrand_region' ),
			'addressCountry'  => 'US',
		],
	...
	$phone = newbrand_setting( 'newbrand_phone' );
	if ( $phone ) {
		$node['telephone'] = $phone;
	}

	$email = newbrand_setting( 'newbrand_email' );
	if ( $email ) {
		$node['email'] = $email;
	}

	$instagram = newbrand_setting( 'newbrand_instagram' );
	if ( $instagram ) {
		$node['sameAs'] = [ $instagram ];
	}

	$founded = newbrand_setting( 'newbrand_founded' );
	if ( $founded ) {
 ...
 */
function newbrand_walk_blocks( array $blocks, callable $visit ): void {
	foreach ( $blocks as $block ) {
		$visit( $block );
		if ( ! empty( $block['innerBlocks'] ) ) {
			newbrand_walk_blocks( $block['innerBlocks'], $visit );
		}
	}
}
 ...
 */
function newbrand_faq_from_blocks( string $content ): array {
 ...
 */
function newbrand_image_ids( int $post_id ): array {
	$ids     = [];
 ...
 */
function newbrand_breadcrumb_node(): ?array {
...
add_action(
	...
	function () {
		...
		$breadcrumb = newbrand_breadcrumb_node();
		if ( $breadcrumb ) {
			$graph[] = $breadcrumb;
		}
		...
		if ( is_singular() ) {
			...
			$faq = newbrand_faq_from_blocks( $content );
			if ( $faq ) {
			...
			if ( is_page() && 'page-service' === get_page_template_slug( $post_id ) ) {
				$price = newbrand_setting( 'newbrand_price_from' );
				$node  = [
					'@type'       => 'Service',
					'name'        => get_the_title(),
					...
					'areaServed'  => [ '@type' => 'City', 'name' => newbrand_setting( 'newbrand_city' ) ],
				];
				if ( $price ) {
					$node['offers'] = [
						'@type'         => 'Offer',
			...
			if ( is_singular( 'session' ) ) {
				$images = array_slice( newbrand_image_ids( $post_id ), 0, 20 );
				if ( $images ) {
					$graph[] = [
						'@type'           => 'ImageGallery',
						'name'            => get_the_title(),
						...
						'image'           => array_values( array_filter( array_map(
							static fn( $id ) => wp_get_attachment_image_url( $id, 'newbrand-full' ),
							$images
						) ) ),
						'author'          => [ '@id' => home_url( '/#business' ) ],
						'contentLocation' => get_post_meta( $post_id, 'newbrand_location', true )
							?: newbrand_setting( 'newbrand_city' ) . ', ' . newbrand_setting( 'newbrand_region' ),
					];
				}
			}
		}
...
add_action(
	...
	function () {
		...
		if ( is_singular() && has_post_thumbnail() ) {
			$image = (string) get_the_post_thumbnail_url( null, 'newbrand-hero' );
		}
		...
		printf( "<meta property=\"og:site_name\" content=\"%s\">\n", esc_attr( newbrand_setting( 'newbrand_studio_name' ) ) );
