<?php
/**
 * Server render for newbrand/breadcrumbs.
 *
 * Mirrors newbrand_breadcrumb_node() in inc/seo-schema.php exactly. Google
...
$crumbs = [ [ 'label' => __( 'Home', 'newbrand' ), 'url' => home_url( '/' ) ] ];

if ( is_singular( 'session' ) ) {
	$archive = get_post_type_archive_link( 'session' );
	if ( $archive ) {
		$crumbs[] = [ 'label' => __( 'Portfolio', 'newbrand' ), 'url' => $archive ];
	}
...
$wrapper = get_block_wrapper_attributes( [ 'class' => 'nb-breadcrumbs' ] );
...
printf(
	...
	esc_attr__( 'Breadcrumb', 'newbrand' ),
