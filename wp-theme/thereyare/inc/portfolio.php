...
add_action(
	...
	function () {
		register_post_type(
			...
			[
				'labels'        => [
					'name'          => __( 'Sessions', 'thereyare' ),
					'singular_name' => __( 'Session', 'thereyare' ),
					'add_new_item'  => __( 'Add new session', 'thereyare' ),
					'edit_item'     => __( 'Edit session', 'thereyare' ),
					'all_items'     => __( 'All sessions', 'thereyare' ),
					'menu_name'     => __( 'Portfolio', 'thereyare' ),
				],
				'public'        => true,
		...
		register_taxonomy(
			...
			[
				'labels'            => [
					'name'          => __( 'Session types', 'thereyare' ),
					'singular_name' => __( 'Session type', 'thereyare' ),
				],
				'hierarchical'      => true,
				'public'            => true,
...
add_filter(
	...
	function ( array $columns ): array {
		$columns['thereyare_location'] = __( 'Location', 'thereyare' );
		return $columns;
	}
);
...
add_action(
	...
	function ( string $column, int $post_id ): void {
		if ( 'thereyare_location' !== $column ) {
			return;
		}
		$location = get_post_meta( $post_id, 'thereyare_location', true );
		echo $location
			? esc_html( $location )
			: '<span style="color:#b32d2e">' . esc_html__( 'Missing', 'thereyare' ) . '</span>';
	},
	10,
	2
);
